...
export function CategoryForm({ categoryId }: { categoryId?: number }) {
  ...
  useEffect(() => { if (!categoryId) return; void adminService.getCategory(categoryId).then((result) => { if (result.error) setError(result.error.message); else if (result.data) setName(result.data.name); else setError("Category not found."); setLoading(false); }); }, [categoryId]);
